...
public class BankApp {
    ...
    public static void main(String[] args) {
        ...
        // вывод очереди сортированной по умолчанию с учётом
        // приоритетных услуг (Вложение средств или Открытие депозита)
        printQueue(queueTicket);

        // Когда наступает время обслуживания пенсионеров.
        // Включаем в компараторе сортировку по времени для пенсионеров.
        // И пересчитываем порядок очереди
...
class TicketComparator implements Comparator<Ticket> {
    ...
    public int compare(Ticket t1, Ticket t2) {
        ...
        // ПРОВЕРКА ПЕНСИОННОГО ВОЗРАСТА ПО ВРЕМЕНИ
        if (isPensionerTime) {
            // Если у обоих клиентов пенсионный возраст, то проверка пенсионеров переходит на проверку услуг
